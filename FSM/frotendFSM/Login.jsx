...
const checkRePasswordEntered=()=>{
  ...
  const submitEmail = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:9000/emailcheck", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.present) {
        ...
        try {
          alert("Sending mail...");
          const emailRes = await fetch("http://localhost:9000/sendemail", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email })
          });
          ...
          alert("Error while sending OTP email.");
        }
      }
    } catch (error) {
  ...
  const submitOtp = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:9000/otpverify", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        ...
        alert("otp is successfully verified") // OTP is verified
      } else {
...
>
  ...
  type="submit"
  className={`bg-blue-800 text-white px-6 py-2 rounded-full font-semibold transition duration-300 ease-in-out transform 
  ${!isEmailVerified && !isOtpVerified&& !isPasswordCheck && !isPasswordEntered && !isUsernameCheck? 'cursor-not-allowed opacity-50' : 'hover:scale-105'}`}
  disabled={!isEmailVerified && !isOtpVerified && !isPasswordCheck && !isPasswordEntered && !isUsernameCheck}
>
  {/* Disabled symbol (lock icon) */}
  {/* <span className={`${!isEmailVerified || !isOtpVerified ? 'block' : 'hidden'} mr-2`}/> */}
{/* /  Submit
</button> */} 
